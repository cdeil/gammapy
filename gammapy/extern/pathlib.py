...
def _py2_fsencode(parts):
    ...
    assert six.PY2
    return [
        part.encode('ascii') if isinstance(part, six.text_type) else part
        for part in parts
    ]
...
def _win32_get_unique_path_id(path):
    ...
    class FILETIME(Structure):
        _fields_ = [("datetime_lo", DWORD), ("datetime_hi", DWORD)]
    ...
    if os.path.isdir(path):
        ...
        flags = 0
    hfile = CreateFile(
        path, GENERIC_READ, FILE_SHARE_READ, None, OPEN_EXISTING, flags, None
    )
    if hfile == 0xffffffff:
        if sys.version_info >= (3, 3):
    ...
    def mkdir(self, mode=0o777, parents=False, exist_ok=False):
        ...
        if not parents:
            _try_except_fileexistserror(
                lambda: self._accessor.mkdir(self, mode), helper
            )
        else:
            try:
                _try_except_fileexistserror(
