...
from gammapy.spectrum.flux_point import compute_differential_flux_points, _energy_lafferty_power_law, _x_lafferty, _integrate
...
def plot_plaw():
    ...
    energies = np.array(_x_lafferty(energy_min, energy_max, spectral_model_function))
    diff_fluxes = spectral_model_function(energies)
    indices = np.array([0, 1, 2, 3, 4])
    int_flux = power_law_integral_flux(diff_fluxes, (indices+1), energies, energy_min, energy_max)
    special = lambda x: np.log(x)
    #import IPython; IPython.embed()
    int_flux[0] = np.abs(_integrate([energy_min[0]], [energy_max[0]], special)[0])
    # Put data into table
    table = Table()
...
def compute_flux_residuals(gamma_true, gamma_reco, method):
    ...
    int_flux = power_law_integral_flux(diff_flux, gamma_true, energies, energy_min, energy_max)
    ...
    for index in indices:
        ...
        lafferty, log = get_flux_tables(table, 'power_law', None, gamma_reco[index])
...
def residuals_image():
    ...
    residuals_lafferty = compute_flux_residuals(gamma_true, gamma_reco, method='lafferty')
    residuals_logcenter = compute_flux_residuals(gamma_true, gamma_reco, method='log_center')
    residuals_ratio = compute_flux_residuals(gamma_true, gamma_reco, method='ratio')
    f, axarr = plt.subplots(nrows=1, ncols=3)
    axarr.flat[0].imshow(residuals_lafferty, interpolation='nearest')
    axarr.flat[0].set_xlabel('Gamma_guess')
    axarr.flat[0]. set_ylabel('Gamma_true')
    ...
    #import IPython; IPython.embed()
    #f.colorbar(axarr[0], cax=cbar_ax)
